# Public demo deployment on Google Cloud Run — design

Date: 2026-09-18. Status: approved (revised: Hugging Face now requires PRO for Docker Spaces; target moved to Cloud Run).

## Goal

A free-tier, always-linkable public demo of NipunyaMatch where visitors bring their own Gemini
API key, hosted on Google Cloud Run (scale-to-zero). First deploy is manual from the laptop;
GitHub `main` stays the source of truth.

## Constraints

- Free tier only. Cloud Run: 2M requests, 360k GB-s memory, 180k vCPU-s per month free in
  `us-central1`; one container, one `$PORT`, ephemeral disk, scale to zero (cold start 30–60 s
  with the torch image). Billing account must exist on the project but stays at $0 at demo traffic.
...
OpenRouter fallback for visitors, HF/Render/Fly/Streamlit Cloud targets, CI-driven deploys (manual `gcloud run deploy` for now).
...
**Settings.** `GEMINI_API_KEY` stays required at startup (fail-fast contract unchanged). On
Cloud Run it is set to a placeholder and is never used for visitor traffic.
...
- `entrypoint.sh` (repo root, executable): if `data/recruiter.db` is absent run
  ...
  the background; exec `streamlit run app/ui/Home.py --server.port ${PORT:-8080}
  --server.address 0.0.0.0 --server.headless true`. `API_BASE_URL=http://127.0.0.1:8000` exported
  for the UI. Cloud Run injects `PORT=8080`.
- `Dockerfile`: keep `tesseract-ocr`; pre-download `all-MiniLM-L6-v2` at build
  (`HF_HOME=/app/.cache`); `CMD ["./entrypoint.sh"]`. `docker-compose.yml` keeps its two-service
  layout for local use by setting `command` per service.
- Cloud Run service settings that matter: `--memory 2Gi --cpu 1 --min-instances 0
  --max-instances 1 --concurrency 20 --timeout 3600 --session-affinity --no-cpu-throttling`.
  `--timeout 3600` + session affinity keep Streamlit's WebSocket alive; `--no-cpu-throttling`
  lets `BackgroundTasks` (resume batches) keep running after the upload response returns.
  `--max-instances 1` because SQLite is per-instance.

## 4. Deploy pipeline

- One-time: install Google Cloud SDK (`winget install Google.CloudSDK`), `gcloud auth login`,
  create project `nipunyamatch-demo`, attach billing, enable `run.googleapis.com`,
  `cloudbuild.googleapis.com`, `artifactregistry.googleapis.com`.
- Deploy: `gcloud run deploy nipunyamatch --source . --region us-central1 --allow-unauthenticated
  --set-env-vars GEMINI_API_KEY=demo-unused,DEMO_MODE=true,MAX_CONCURRENT_LLM=2` plus the
  settings above. Cloud Build builds the Dockerfile remotely (no local Docker needed).
- Redeploy = rerun the same command. Wrapped in `scripts/deploy.sh` so it is one line.
- The maintainer creates the project and billing; everything else is scripted.
...
Manual: `docker build -t nm .` then run with `-p 8080:8080 -e GEMINI_API_KEY=x -e DEMO_MODE=true`
(or skip local Docker and use the Cloud Run URL); open the UI, enter a real key in the sidebar,
run Setup → Processing → Candidates → Assistant.
...
`README.md`, `scripts/deploy.sh`, `.env.example` (`DEMO_MODE=false`), tests.
