# Cloud Run Public Demo Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship NipunyaMatch as a free-tier public demo on Google Cloud Run where visitors use their own Gemini key; one-command redeploy from the laptop.

**Architecture:** One container runs uvicorn (127.0.0.1:8000) and Streamlit (0.0.0.0:$PORT, 8080 on Cloud Run). The API resolves a per-visitor `Gateway` from an `X-Gemini-Key` header (bounded in-process pool); `DEMO_MODE=true` turns off deletes/rescore and caps uploads. `scripts/deploy.sh` wraps `gcloud run deploy --source .` (Cloud Build builds the image remotely).

**Tech Stack:** existing FastAPI + Streamlit + SQLite app; Docker; Google Cloud Run + Cloud Build; gcloud CLI.

**Spec:** `docs/superpowers/specs/2026-09-18-cloud-run-demo-deploy-design.md`

## Global Constraints

- `GEMINI_API_KEY` stays required at startup (Settings fail-fast unchanged). On Cloud Run it is a placeholder.
...
- Single exposed port `$PORT` (8080 on Cloud Run); SQLite ephemeral, seeded on boot by `scripts/seed.py` (no LLM); `--max-instances 1`.
...
**Interfaces:**
- Produces: image serving Streamlit on `$PORT` (default 8080) with the API on 127.0.0.1:8000.
...
Keeps the Cloud Build upload small (the `.venv` alone is >1 GB):
```
.venv
.git
data
...
- [ ] **Step 5: Smoke locally (only if Docker is installed; otherwise Task 6 is the test)**

Run: `docker build -t nipunyamatch . && docker run --rm -p 8080:8080 -e GEMINI_API_KEY=demo-unused -e DEMO_MODE=true nipunyamatch`
Expected: log shows "seeding demo data", the 12-row ranked table, then the Streamlit URL; open
http://localhost:8080 — sidebar banner + key field; Candidates shows the seeded job.

- [ ] **Step 6: Commit**

```bash
git add entrypoint.sh Dockerfile docker-compose.yml .dockerignore
git update-index --chmod=+x entrypoint.sh
git commit -m "build: single-container entrypoint for Cloud Run, baked MiniLM, dockerignore"
```

---

### Task 6: Cloud Run deploy script and README

**Files:**
- Create: `scripts/deploy.sh`
- Modify: `README.md` (Live demo line under Demo; deploy line in §8)

**Interfaces:**
- Consumes: gcloud CLI authenticated against a project with billing; env `GCP_PROJECT`, `GCP_REGION`.

- [ ] **Step 1: One-time manual setup (maintainer)**

```powershell
winget install Google.CloudSDK          # then open a new terminal
gcloud auth login
gcloud projects create nipunyamatch-demo --name "NipunyaMatch demo"
gcloud config set project nipunyamatch-demo
# attach billing in the console: https://console.cloud.google.com/billing/linkedaccount?project=nipunyamatch-demo
...
```
`--max-instances 1` because SQLite lives inside the instance; `--no-cpu-throttling` so resume
batches keep processing after the upload response; `--timeout 3600` + `--session-affinity` for
Streamlit's WebSocket.

- [ ] **Step 3: Deploy**

Run: `sh scripts/deploy.sh` (Git Bash). First build ~8–12 min (torch). Expected: prints the
service URL. Open it: banner + key field; Candidates shows the seeded job; enter a real Gemini
key, ask a question.

- [ ] **Step 4: README**

Under `## Demo`, after the GIF paragraph, add (paste the real URL from Step 3):
```markdown
**Live demo:** <service URL> — bring your own free Gemini key (sidebar). Cold start takes
~30–60 s after idle. Data is shared between visitors and resets on restart; deletes are disabled.
```
In `## 8. How to run` add to the command block:
```
sh scripts/deploy.sh                   # (re)deploy the public demo to Cloud Run
```
...
git commit -m "ops: one-command Cloud Run deploy for the public demo"
```
...
§3 container + Cloud Run settings → Tasks 5, 6. §4 pipeline → Task 6. §5 tests → Tasks 1–3 (`test_demo.py`), Task 4
